...
# Pretrained speaker model (ECAPA-TDNN) from speechbrain
...
def get_args():
    ...
    parser.add_argument('--device', default='cuda',
                        help='inference device type: cpu, cuda, ipu, xpu, \
                        mkldnn, opengl, opencl, ideep, hip, ve, ort, mlc, \
...
def compute_embeddings(scp, segments, source, device,
    ...
    def init_encoder(source, device):
        encoder = EncoderClassifier.from_hparams(source=source,
                                                 savedir='/tmp/' + source,
                                                 run_opts={"device": device})

        return encoder

    def extract_embeddings(x, batch_size):
        embeddings = []
        for i in range(0, x.size(0), batch_size):
            batch_x = x[i: i + batch_size, :]
            batch_y = encoder.encode_batch(batch_x).cpu().squeeze().numpy()
            embeddings.append(batch_y)
